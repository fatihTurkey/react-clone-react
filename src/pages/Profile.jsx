  ...
  async function onSubmit() {
    try {
      if (auth.currentUser.displayName !== name) {
        // update the display name firebase auth
        await updateProfile(auth.currentUser, {
          displayName: name,
        })
        // update the name in te firestore
    ...
    <>
      <section className="max-w-6xl mx-auto flex justify-center items-center flex-col">
        ...
        <div className="w-full md:w-[50%] mt-6 px-3">
          <form>
            <input type="text" id="name" value={name} className={`mb-6 w-full px-4 py-2 text-xl text-gray-700 bg-white border border-gray-300 rounded transition ease-in-out ${changeDetail && "bg-red-100 focus:bg-red-200"}`} disabled={!changeDetail} onChange={onChange} />
